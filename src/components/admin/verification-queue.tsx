...
export function VerificationQueue() {
  ...
  async function load() { setState("loading"); try { const response = await fetch(new URL("/api/verifications", window.location.origin), { cache: "no-store" }); const body = await response.json() as { verifications?: Row[]; error?: string }; if (!response.ok) throw new Error(body.error || "Database verifikasi belum tersedia."); const next = body.verifications ?? []; setRows(next); setState(next.length ? "ready" : "empty"); } catch (error) { setMessage(error instanceof Error ? error.message : "Database verifikasi belum tersedia."); setState("error"); } }
  useEffect(() => { const timer = window.setTimeout(() => { void load(); }, 0); return () => window.clearTimeout(timer); }, []);
  async function update(id: string, status: string) { setBusy(id); setMessage(""); const response = await fetch(new URL(`/api/verifications/${id}`, window.location.origin), { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); if (!response.ok) setMessage(((await response.json()) as { error?: string }).error || "Status gagal diperbarui."); else await load(); setBusy(null); }
