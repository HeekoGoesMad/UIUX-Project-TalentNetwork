...
export function ProtectedRoute({ children, role }: { children: ReactNode; role: UserRole }) {
  ...
  if (!hydrated || (dbMode && !bootstrapped) || !user || user.role !== role || (role === "recruiter" && user.provisioningStatus !== "active")) return <div className="mx-auto max-w-md px-4 py-24 text-center"><div className="mx-auto size-8 animate-pulse rounded-full bg-[#d7f5e8]" /><p className="mt-4 text-sm text-muted-foreground">Menyiapkan workspace...</p></div>;
  return <>{children}</>;
}
