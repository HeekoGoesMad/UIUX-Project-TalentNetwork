...
export function CandidateReviewStatus({
  invitationId,
  compact = false,
  initialReview,
}: {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const list = await fetch("/api/assessment-invitations", { cache: "no-store" })
  ...
  }, [dbMode, invitationId, initialReview]);
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {review.status === "completed" && review.score !== null && (
              <p className="mt-3 text-sm">Score keseluruhan: <strong>{review.score}/100</strong></p>
            )}
            <p className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
              <MessageSquareText className="size-3.5" /> Catatan internal recruiter tidak ditampilkan.
            </p>
          </div>
        </div>
      </CardContent>
...
export function CandidateReviewList() {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!dbMode) {
        setItems(listDemoInvitations().map((item) => ({ id: item.id, review: item.attemptId ? getDemoReview(item.attemptId) : null })));
        return;
      }
      const list = await fetch("/api/assessment-invitations", { cache: "no-store" })
        .then((response) => response.json()) as {
      ...
      if (active) {
        setItems((list.invitations ?? []).map((item) => ({
          id: item.id,
          review: item.attempt?.review ?? null,
        })));
      }
    }
    void load();
  ...
  return (
    <div className="mt-4 space-y-3">
      {items.filter((item) => item.review).map((item) => (
        <CandidateReviewStatus key={item.id} compact invitationId={item.id} initialReview={item.review} />
      ))}
    </div>
  );
}
