...
const labels: Record<ApplicationStatus, string> = { new: "New", shortlisted: "Shortlisted", consent_requested: "Consent requested", consent_approved: "Consent approved", screening: "Screening", assessment: "Assessment", review: "Review", interview: "Interview", offer: "Offer", hired: "Hired", rejected: "Rejected", withdrawn: "Withdrawn" };
const activeStatuses = applicationStatuses.filter((status) => status !== "withdrawn");
const stageColors: Record<ApplicationStatus, string> = { new: "bg-slate-100 text-slate-700", shortlisted: "bg-purple-100 text-purple-800", consent_requested: "bg-amber-100 text-amber-800", consent_approved: "bg-emerald-100 text-emerald-800", screening: "bg-blue-100 text-blue-800", assessment: "bg-indigo-100 text-indigo-800", review: "bg-cyan-100 text-cyan-800", interview: "bg-fuchsia-100 text-fuchsia-800", offer: "bg-emerald-100 text-emerald-800", hired: "bg-emerald-200 text-emerald-900", rejected: "bg-red-100 text-red-800", withdrawn: "bg-slate-200 text-slate-700" };

const storageKey = "proofylink-demo-applications";
function demoApplications(): Application[] { try { return JSON.parse(localStorage.getItem(storageKey) ?? "[]") as Application[]; } catch { return []; } }
...
function useApplications() {
  ...
  return { applications, setApplications, loading, error, dbMode };
}

export function CandidateApplicationsPage() {
  const { applications, loading, error, dbMode } = useApplications();
  return <ProtectedRoute role="candidate"><main className="container mx-auto max-w-5xl px-4 py-8 sm:py-12"><Link href="/candidate" className="inline-flex items-center gap-2 text-sm font-semibold text-primary"><ArrowLeft className="size-4" /> Candidate workspace</Link><div className="mt-7 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="font-mono text-xs uppercase tracking-widest text-primary">Candidate workspace</p><h1 className="mt-2 text-3xl font-bold">Aplikasi saya</h1><p className="mt-2 text-muted-foreground">Pantau setiap langkah dari lamaran sampai keputusan akhir.</p></div><Button asChild variant="outline"><Link href="/jobs"><Send className="size-4" /> Cari lowongan</Link></Button></div>{!dbMode && <p className="mt-5 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">Mode demo: aplikasi hanya tersimpan di browser ini dan tidak membuat data palsu.</p>}<div className="mt-8">{loading ? <State text="Memuat aplikasi..." /> : error ? <State text={error} error /> : applications.length === 0 ? <State text="Belum ada aplikasi. Temukan lowongan yang sesuai dan kirim lamaran pertamamu." /> : <div className="space-y-4">{applications.map((application) => <Link key={application.id} href={`/candidate/applications/${application.id}`} className="block"><Card className="transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-lg font-semibold">{application.job?.title ?? "Job"}</p><p className="mt-1 text-sm text-muted-foreground">{application.job?.organizationName ?? "Organisasi"}</p><p className="mt-3 font-mono text-xs text-muted-foreground">Dikirim {formatDate(application.submittedAt)}</p></div><div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end">{statusBadge(application.status)}<span className="text-xs font-semibold text-primary">Lihat detail -&gt;</span></div></CardContent></Card></Link>)}</div>}</div></main></ProtectedRoute>;
}
