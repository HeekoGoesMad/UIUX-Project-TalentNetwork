...
export function getProductionConfig(): ProductionConfig {
  ...
  return {
    ...
    readiness: {
      ...
      email: !flags.emailDeliveryEnabled || (hasValue("EMAIL_PROVIDER") && hasValue("EMAIL_WEBHOOK_SECRET")),
    },
  };
}
...
export function validateProductionConfig() {
  ...
  if (!config.readiness.email) missing.push("EMAIL_PROVIDER and EMAIL_WEBHOOK_SECRET");
  if (config.flags.emailDeliveryEnabled && process.env.EMAIL_PROVIDER?.trim().toLowerCase() === "mock") missing.push("EMAIL_PROVIDER must not be mock");
