...
export default function Page() {
  ...
  return <ProtectedRoute role="recruiter"><main className="container mx-auto max-w-3xl px-4 py-8">
    ...
    <Card className="mt-8 overflow-hidden"><CardContent className="space-y-6 p-6">
      ...
      <div className="rounded-xl border border-[#bcebd8] bg-[#f3fcf7] p-4 text-sm"><p className="font-semibold text-[#08744f]">Consent: {consent === "pending-candidate-consent" ? "Menunggu kandidat" : consent === "consented" ? "Disetujui" : consent === "screening-completed" ? "Screening selesai" : consent === "declined" ? "Ditolak" : "Belum diminta"}</p><p className="mt-1 text-muted-foreground">Financial, credit, dan atribut sensitif tidak digunakan.</p></div>
      ...
      {consent === "screening-completed" && <p className="text-sm font-semibold text-[#08744f]">Screening selesai. Token tidak akan terpotong lagi untuk screening ini.</p>}
    </CardContent></Card>
  </main></ProtectedRoute>;
}
