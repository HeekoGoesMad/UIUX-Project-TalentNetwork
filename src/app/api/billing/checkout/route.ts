...
export async function POST(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Checkout belum tersedia.";
    return NextResponse.json({ error: message }, { status: message.includes("PAYMENT_PROVIDER") ? 503 : 500 });
  }
}
