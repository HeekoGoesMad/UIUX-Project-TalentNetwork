  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (paidEvents.has(body.type!) && purchase.status !== "paid") {
        ...
        if (entry) await tx.update(schema.tokenAccounts).set({ balance: account.balance + purchase.tokenAmount, updatedAt: new Date() }).where(eq(schema.tokenAccounts.id, account.id));
