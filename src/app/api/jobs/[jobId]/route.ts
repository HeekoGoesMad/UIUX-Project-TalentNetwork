...
const updateSchema = z.object({
  ...
  salaryPeriod: z.enum(["monthly", "hourly", "yearly"]).optional(),
  ...
  vacanciesCount: z.number().int().min(1).optional(),
...
async function load(db: Awaited<ReturnType<typeof import("@/db").getDb>>, jobId: string, organizationId?: string) {
  const rows = await db
    .select({
      ...
      requirement: schema.jobRequirements,
    })
    .from(schema.jobs)
    ...
    .where(
      and(
        ...
        organizationId ? eq(schema.jobs.organizationId, organizationId) : eq(schema.jobs.status, "published")
      )
    );
  ...
  const first = rows[0];
  const reqMap = new Map<string, { id: string; type: "required" | "preferred"; name: string }>();
  for (const row of rows) {
    if (row.requirement) {
      reqMap.set(row.requirement.id, {
  ...
  return {
    ...
    organization: {
      id: first.org.id,
      name: first.org.name,
      logoUrl: first.org.logoUrl,
      bannerUrl: first.org.bannerUrl,
      description: first.org.description,
      officeAddress: first.org.officeAddress,
      city: first.org.city,
      province: first.org.province,
      companyEmail: first.org.companyEmail,
      companyPhone: first.org.companyPhone,
      website: first.org.website,
      linkedinUrl: first.org.linkedinUrl,
      industry: first.org.industry,
      companyScale: first.org.companyScale,
      verificationStatus: first.org.verificationStatus,
    },
...
export async function GET(_request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const current = await import("@/lib/api/auth").then(({ getCurrentAppUser }) => getCurrentAppUser());
...
export async function PATCH(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "Data job tidak valid." }, { status: 400 });
    ...
    if (next && next !== existing[0].status && !allowedTransitions[existing[0].status]?.includes(next)) {
      return NextResponse.json({ error: `Transisi status ${existing[0].status} ke ${next} tidak diizinkan.` }, { status: 409 });
    }
    ...
    if (!updated) return NextResponse.json({ error: "Job tidak ditemukan." }, { status: 404 });

    if (requiredSkills || preferredSkills) {
      await current.db.delete(schema.jobRequirements).where(eq(schema.jobRequirements.jobId, jobId));
      const requirements = [
        ...(requiredSkills ?? []).map((name) => ({ jobId, name, type: "required" as const })),
        ...(preferredSkills ?? []).map((name) => ({ jobId, name, type: "preferred" as const })),
      ];
