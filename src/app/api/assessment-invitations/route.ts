...
export async function POST(request: Request) {
  try {
    ...
    const result = await current.db.transaction(async (tx) => {
       const rows = await tx.select({ application: schema.applications, jobTitle: schema.jobs.title, organizationId: schema.jobs.organizationId, candidateUserId: schema.candidateProfiles.userId }).from(schema.applications).innerJoin(schema.jobs, eq(schema.jobs.id, schema.applications.jobId)).innerJoin(schema.candidateProfiles, eq(schema.candidateProfiles.id, schema.applications.candidateProfileId)).where(eq(schema.applications.id, parsed.data.applicationId)).limit(1);
      ...
      const [template] = await tx.select({ id: schema.assessmentTemplates.id, name: schema.assessmentTemplates.name, organizationId: schema.assessmentTemplates.organizationId }).from(schema.assessmentTemplates).where(eq(schema.assessmentTemplates.id, parsed.data.assessmentTemplateId)).limit(1);
      ...
      if (existing) return { error: "Invitation untuk aplikasi dan template ini sudah aktif.", status: 409 as const };
        ...
        const testName = template.name ? ` (${template.name})` : "";
        await createNotificationWithDeliveries(tx, systemNotification({ userId: application.candidateUserId, title: `Undangan Asesmen: ${application.jobTitle || "Kompetensi"}`, body: `Anda diundang untuk menyelesaikan asesmen kompetensi${testName}. Periksa instruksi dan batas waktu pengerjaan.`, data: notificationData(`assessment-invitation:${invitation.id}:created`, `/candidate/assessments/${invitation.id}`, { invitationId: invitation.id, status: "pending" }) }));
       return { invitation };
    });
