...
function navigateToConversation(router: { replace: (href: string) => void }, pathname: string | null, currentQueryConversationId: string | null, conversationId: string): void {
  ...
  if (base !== "/messages") {
    ...
    router.replace(`${base}?conversationId=${encodeURIComponent(conversationId)}`);
    return;
  }
...
function MessagesContent({ routeConversationId }: { routeConversationId?: string }) {
  ...
  const pageBody = (
    <div className={cn("space-y-6", !isEmbedded && "mx-auto max-w-6xl space-y-8")}>
      <div>
        <h1 className={cn("text-2xl font-bold tracking-tight text-foreground sm:text-3xl", !isEmbedded && "mt-2")}>Pesan</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Percakapan hanya tersedia selama consent aktif dan masa penyimpanan belum berakhir.</p>
      </div>
      {visibleError && (
      ...
      <div className="grid gap-6 lg:grid-cols-[20rem_1fr]">
        <Card className="overflow-hidden"><CardHeader className="border-b bg-muted/50"><CardTitle className="flex items-center gap-2 text-lg"><MessageCircle className="size-4 text-muted-foreground" /> Percakapan</CardTitle></CardHeader><CardContent className="space-y-1 p-2">
          {loading ? <p role="status" className="p-4 text-sm text-muted-foreground">Memuat percakapan...</p> : databaseMode && conversations.length === 0 ? <EmptyState icon={MessageCircle} title="Belum ada percakapan." className="border-0 bg-transparent p-6 shadow-none" /> : (databaseMode ? conversations : [{ id: "demo", status: "active", updatedAt: "", participants: [{ id: "other", name: "Nadia Pratama" }], lastMessage: { body: "Percakapan demo untuk pratinjau", createdAt: "" } }]).map((conversation) => { const participant = conversation.participants.find((item) => item.id !== currentUserId && item.email !== userEmail); return <button type="button" key={conversation.id} onClick={() => { setSelectedId(conversation.id); if (databaseMode) navigateToConversation(router, pathname, currentQueryConversationId, conversation.id); }} className={cn("w-full rounded-xl p-4 text-left transition-colors hover:bg-muted/50", selectedId === conversation.id && "bg-muted")}><p className="font-semibold">{participant?.name ?? "Kontak"}</p><p className="mt-1 truncate text-sm text-muted-foreground">{conversation.lastMessage?.body ?? "Belum ada pesan"}</p></button>; })}
        </CardContent></Card>
        <Card className="flex min-h-[32rem] min-w-0 flex-col"><CardHeader className="border-b"><div className="flex flex-wrap items-start justify-between gap-3"><CardTitle className="flex min-w-0 items-center gap-2 text-lg"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground"><ShieldCheck className="size-4" /></span><span className="truncate">{other}</span></CardTitle>{databaseMode && selected && <Button type="button" variant="outline" size="sm" onClick={() => void toggleBlock()} disabled={blockLoading} aria-label={selected.status === "blocked" ? "Buka blokir percakapan" : "Blokir percakapan"} className="shrink-0">{blockLoading ? <LoaderCircle className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />}<span>{selected.status === "blocked" ? "Buka blokir" : "Blokir"}</span></Button>}</div>{databaseMode && selected?.status === "blocked" && <p role="status" className="text-xs font-medium text-amber-700">Percakapan diblokir. Buka blokir untuk mengirim pesan kembali.</p>}</CardHeader><CardContent className="flex min-h-0 flex-1 flex-col gap-4 p-4 sm:p-6"><div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">{messagesLoading && messages.length === 0 ? <p role="status" className="py-8 text-center text-sm text-muted-foreground"><LoaderCircle className="mx-auto mb-2 size-5 animate-spin" />Memuat pesan...</p> : databaseMode && !selected ? <p className="py-8 text-center text-sm text-muted-foreground">Pilih percakapan untuk melihat pesan.</p> : messages.length === 0 ? <EmptyState icon={MessageCircle} title="Belum ada pesan dalam percakapan ini." className="border-0 bg-transparent shadow-none" /> : <>{hasMore && nextCursor && <div className="flex justify-center pb-1"><Button type="button" variant="outline" size="sm" onClick={() => void loadMessages(nextCursor)} disabled={messagesLoading}>{messagesLoading ? <LoaderCircle className="size-4 animate-spin" /> : null}{messagesLoading ? "Memuat pesan..." : "Muat pesan sebelumnya"}</Button></div>}{messages.map((message) => { const mine = message.isMine ?? message.senderId === currentUserId; const actionPending = actionLoading?.startsWith(`${message.id}:`) ?? false; const actionMenu = databaseMode ? <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon" aria-label={`Aksi untuk pesan ${mine ? "Anda" : "ini"}`} disabled={Boolean(actionLoading)} className="mt-1 size-8 shrink-0 text-muted-foreground opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">{actionPending ? <LoaderCircle className="size-4 animate-spin" /> : <MoreHorizontal className="size-4" />}</Button></DropdownMenuTrigger><DropdownMenuContent align={mine ? "end" : "start"}>{mine ? <><DropdownMenuItem onSelect={() => void messageAction(message, "edit")}><Pencil className="size-4" />Edit pesan</DropdownMenuItem><DropdownMenuItem destructive onSelect={() => void messageAction(message, "delete")}><Trash2 className="size-4" />Hapus pesan</DropdownMenuItem></> : <DropdownMenuItem onSelect={() => void messageAction(message, "report")}><Flag className="size-4" />Laporkan pesan</DropdownMenuItem>}</DropdownMenuContent></DropdownMenu> : null; return <div key={message.id} className={cn("group flex items-start gap-2", mine ? "justify-end" : "justify-start")}>{!mine && actionMenu}<div className={cn("max-w-[85%] min-w-0 rounded-2xl px-4 py-3 text-sm leading-6", mine ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md bg-muted text-foreground")}><p className="whitespace-pre-wrap break-words">{message.body}</p>{message.attachmentName && <div className={cn("mt-2 flex items-center justify-between gap-3 rounded-xl border p-2.5 text-xs", mine ? "border-white/20 bg-white/10 text-white" : "border-border bg-white text-foreground")}><div className="flex min-w-0 items-center gap-2"><FileText className="size-4 shrink-0 opacity-80" /><div className="min-w-0"><p className="truncate font-medium">{message.attachmentName}</p>{message.attachmentSize && <p className="text-xs opacity-70">{formatBytes(message.attachmentSize)}</p>}{message.attachmentScanStatus && message.attachmentScanStatus !== "not_applicable" && <p className="text-xs opacity-70">{message.attachmentScanStatus === "pending" ? "Memeriksa lampiran…" : message.attachmentScanStatus === "blocked" ? "Lampiran diblokir" : message.attachmentScanStatus === "clean" ? "Lampiran aman" : message.attachmentScanStatus}</p>}</div></div>{databaseMode ? <Button type="button" variant="ghost" size="sm" className={cn("h-7 px-2 text-xs", mine ? "text-white hover:bg-white/20" : "text-emerald-700 hover:bg-emerald-50")} onClick={async () => { try { const response = await fetch(`/api/messages/${encodeURIComponent(message.id)}/attachment`); const payload = await response.json() as { downloadUrl?: string; error?: string }; if (!response.ok || !payload.downloadUrl) { setError(payload.error ?? "Gagal mengunduh lampiran."); return; } window.open(payload.downloadUrl, "_blank", "noopener,noreferrer"); } catch { setError("Gagal mengunduh lampiran."); } }}>Unduh</Button> : <span className="text-xs opacity-60">Pratinjau</span>}</div>}<div className="mt-1 flex items-center justify-end gap-1 text-xs opacity-60"><time dateTime={message.createdAt}>{new Date(message.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</time>{message.editedAt && <span>(diedit)</span>}</div></div>{mine && actionMenu}</div>; })}</>}</div>{attachment && <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/50 px-3 py-1.5 text-xs text-foreground"><FileText className="size-3.5 text-emerald-600" /><span className="max-w-[200px] truncate font-medium">{attachment.name}</span><span className="text-muted-foreground">({formatBytes(attachment.size)})</span><button type="button" onClick={removeAttachment} aria-label="Hapus lampiran" className="ml-1 rounded-sm p-0.5 hover:bg-muted"><X className="size-3.5" /></button></div>}<form onSubmit={sendMessage} className="border-t pt-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/jpeg,image/png,image/gif,image/webp,application/pdf" className="hidden" /><Button type="button" variant="outline" size="icon" aria-label="Lampirkan file" disabled={sending || (databaseMode && (!selectedId || !selected || selected.status === "blocked"))} onClick={() => fileInputRef.current?.click()}><Paperclip className="size-4 text-muted-foreground" /></Button><div className="min-w-0 flex-1">{editingId && <div className="mb-2 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span>Mengedit pesan</span><Button type="button" variant="ghost" size="sm" onClick={() => { setEditingId(null); setDraft(""); }}><X className="size-3.5" />Batal</Button></div>}{editingId && selectedFile && <p className="mb-2 text-xs text-muted-foreground">Lampiran diabaikan saat mengedit pesan.</p>}<Textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Tulis pesan dalam Bahasa Indonesia..." aria-label="Isi pesan" className="min-h-12 resize-none" disabled={sending || (databaseMode && (!selectedId || !selected || selected.status === "blocked"))} /></div><Button type="submit" size="default" aria-label={editingId ? "Simpan perubahan pesan" : "Kirim pesan"} disabled={sending || !draft.trim() || (databaseMode && (!selectedId || !selected || selected.status === "blocked"))} className="w-full shrink-0 sm:w-auto">{sending ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}<span>{sending && selectedFile && !editingId ? "Mengunggah…" : editingId ? "Simpan perubahan" : "Kirim"}</span></Button></div></form></CardContent></Card>
      </div>
    </div>
  );
  ...
  return <main className="container mx-auto px-4 py-8 sm:py-12">{pageBody}</main>;
}

function databaseModeUnavailable(dbMode: boolean, bootstrapped: boolean) { return dbMode && !bootstrapped; }
function StateMessage({ text }: { text: string }) { return <div className="mx-auto max-w-6xl py-8"><div className="rounded-xl border bg-card p-8 text-center text-sm text-muted-foreground" role="status">{text}</div></div>; }
