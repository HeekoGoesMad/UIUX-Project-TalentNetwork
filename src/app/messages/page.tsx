...
import { FileText, MessageCircle, Paperclip, Send, ShieldCheck, X } from "lucide-react";
...
function MessagesContent({ routeConversationId }: { routeConversationId?: string }) {
  ...
  async function sendMessage(event: FormEvent) {
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          ...(attachment ? {
            ...
            attachmentScanStatus: "not_applicable",
          } : {}),
        },
      ]);
      setDraft("");
    ...
    try {
      ...
      if (!response.ok || (!editingId && !result.message)) {
        setError(result.error ?? "Pesan gagal disimpan.");
        return;
      }

      setMessages((current) =>
        editingId
          ? current.map((item) => (item.id === editingId ? { ...item, ...result.message } : item))
  ...
  return <main className="container mx-auto px-4 py-8 sm:py-12"><div className="mx-auto max-w-6xl space-y-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[20rem_1fr]">
      ...
      <Card className="flex min-h-[32rem] flex-col"><CardHeader className="border-b"><CardTitle className="flex items-center gap-2 text-lg"><span className="flex size-9 items-center justify-center rounded-full bg-[#d7f5e8] text-[#08744f]"><ShieldCheck className="size-4" /></span>{other}</CardTitle><p className="text-sm text-muted-foreground">Percakapan hanya tersedia untuk peserta yang berwenang.</p></CardHeader><CardContent className="flex flex-1 flex-col justify-end gap-4 p-4 sm:p-6">
        <div className="space-y-3">
          {hasMore && nextCursor && (
            <div className="flex justify-center pb-2">
              ...
              >
                {messagesLoading ? "Memuat..." : "Muat pesan sebelumnya"}
              </Button>
            </div>
          )}
          {messagesLoading && messages.length === 0 ? <p role="status" className="text-center text-sm text-muted-foreground">Memuat pesan...</p> : databaseMode && !selected ? <p className="text-center text-sm text-muted-foreground">Pilih percakapan untuk melihat pesan.</p> : messages.length === 0 ? <EmptyState icon={MessageCircle} title="Belum ada pesan dalam percakapan ini." className="border-0 bg-transparent shadow-none" /> : messages.map((message) => {
            const mine = message.isMine ?? message.senderId === user.email;
            return <div key={message.id} className={cn("flex", mine && "justify-end")}>
              <div className={cn("max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6", mine ? "rounded-br-md bg-[#0f2040] text-white" : "rounded-bl-md bg-[#f0f6fd] text-[#0a1628]")}>
                <p>{message.body}</p>
                {message.attachmentName && (
                  <div className={cn(
                    "mt-2 flex items-center justify-between gap-3 rounded-xl border p-2.5 text-xs",
                    mine ? "border-white/20 bg-white/10 text-white" : "border-border bg-white text-foreground"
                  )}>
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {message.attachmentSize && (
                          <p className="text-[11px] opacity-70">{formatBytes(message.attachmentSize)}</p>
                        )}
                      </div>
                    </div>
                    {dbMode ? (
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            const res = await fetch(`/api/messages/${encodeURIComponent(message.id)}/attachment`);
                            const data = await res.json() as { downloadUrl?: string; error?: string };
                            if (!res.ok || !data.downloadUrl) {
                              setError(data.error ?? "Gagal mengunduh lampiran.");
                              return;
                            }
                      ...
                      <span className="text-[10px] opacity-60">Pratinjau</span>
                    )}
                  </div>
                )}
                <time className="mt-1 block text-[11px] opacity-60">{new Date(message.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</time>
              </div>
            </div>;
          })}
        </div>
        {attachment && (
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/50 px-3 py-1.5 text-xs text-foreground">
        ...
        <form onSubmit={sendMessage} className="flex items-end gap-2 border-t pt-4">
          ...
          <Button
            ...
            disabled={databaseMode && !selectedId}
            onClick={() => fileInputRef.current?.click()}
          >
            <Paperclip className="size-4 text-muted-foreground" />
          </Button>
          <Textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Tulis pesan dalam Bahasa Indonesia..." aria-label="Isi pesan" className="min-h-12 resize-none" disabled={databaseMode && !selectedId} />
          <Button type="submit" size="icon" aria-label="Kirim pesan" disabled={databaseMode && !selectedId}><Send className="size-4" /></Button>
        </form>
      </CardContent></Card>
    </div>
  </div></main>;
