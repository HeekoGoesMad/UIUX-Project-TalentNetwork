...
function MessagesContent({ routeConversationId }: { routeConversationId?: string }) {
  ...
  return <main className="container mx-auto px-4 py-8 sm:py-12"><div className="mx-auto max-w-6xl space-y-8">
    <div><h1 className="mt-2 text-3xl font-bold tracking-tight text-foreground">Pesan</h1><p className="mt-2 max-w-2xl text-muted-foreground">Percakapan hanya tersedia selama consent aktif dan masa penyimpanan belum berakhir.</p></div>
    {visibleError && <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><span>Gagal memuat atau memperbarui pesan. {visibleError}</span><Button type="button" variant="outline" size="sm" onClick={() => void retryLoad()}>Coba lagi</Button></div>}
